...
const presenterMap = (data) => {
  const resultUserSpecifications = data.map((Temp) => {
    ...
    Temp.idBus ? Temp.idBus = UUIDGenerator.from(Temp.idBus).toString(): '',
    Temp.idRoute? Temp.idRoute = UUIDGenerator.from(Temp.idRoute).toString() : '',
    delete Temp._id
...
const presenterAllReservation = (data) => {
  const resultUserSpecifications = data.map((Temp) => {
    let id = UUIDGenerator.from(Temp._id).toString()
    Temp.idBus ? Temp.idBus = UUIDGenerator.from(Temp.idBus).toString(): '',
    Temp.idRoute? Temp.idRoute = UUIDGenerator.from(Temp.idRoute).toString() : '',
...
export default {
  ...
  presentAllReservation: presenterAllReservation
