...
class Create {
  ...
  async execute(paramDto, id) {
    ...
    const [travel, seat, user] = await Promise.all([
      ...
      this.repository.getSeatByIdUser(idTravel, paramDto.idUser)
      
    ]);

    if (!travel) throw new DataNotFoundException('Travel not found');    
    if (user.length > 0) throw new AlreadyExistsException (`the cpf has already reserved the seat ${user[0].seats.seatNumber}`);
    if (seat.length > 0) throw new AlreadyExistsException ('Already reserved seat');
 
    
    paramDto.reservation_created_at = new Date();
    const result = await this.repository.createReservation(paramDto, idTravel);
    if (!result) throw new InvalidOperationException('Failed to create record.');
